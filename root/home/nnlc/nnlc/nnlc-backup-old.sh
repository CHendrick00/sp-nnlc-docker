  ...
  if [[ -n $files ]]; then
    ...
    while true; do
      ...
      if [[ $INP1 == 'y' ]]; then
        ...
        echo "---------------------------"
        tar -czvf $process_backup_dir/$filename $files latfiles_backup.txt rlogs_backup.txt
        echo
        ...
        echo "Creating backup"
        echo "---------------------------"
        tar -czvf $process_backup_dir/$filename $process_files latfiles_backup.txt rlogs_backup.txt
        echo
        echo "Backup saved to $process_backup_dir/$filename"
      fi
